...
var CODE 
const errSymb = {}; const lblSymb = {}; const iterSymb = {}; const prevSymb = {}
const sigNoWaitSymb = {};
var in_evt = false
var instance

static func nowait_of(sig, args):
	return {sigNoWaitSymb: true, "signal": sig, "args": args}
		...
		if typeof(inst) == TYPE_ARRAY:
			if inst[0] == "LIT": _push(inst[1])
				...
				if in_evt:
					...
					return
				var obj = _pop()
				if sigNoWaitSymb in obj and obj.signal == inst[1]:
					if obj.args != null:
						_push(obj.args)
					IP += 1 
					continue
				elif not obj.is_connected(inst[1], self, "sig_resume"):
			...
			if inst.begins_with("."): 
				...
				if dict.has(word_name):
					...
					halt_fail()
					push_error(str("Tried to exec word '",word_name,"' that doesn't exist!"))
				continue
			elif inst == "do-block":
				_r_push(IP+1)
				if trace_indent: trace += 1
