...
func location_of(mapCoord: Vector2):
    var rs = settings.room_size
    var room_coord = Vector2(mapCoord.x / rs.x, mapCoord.y / rs.y)
    var in_room_coord = Vector2(mapCoord.x % rs.x, mapCoord.y % rs.y)

    return {"map": room_coord, "room": in_room_coord}

func load_map(bgTiles: TileMap, solids: TileMap, settings: GameMap.GameMapSettings):
...
class GameMapSettings extends Reference:
